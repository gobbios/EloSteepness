...
```{r, echo=FALSE}
suppressPackageStartupMessages(library(EloRating))
...
The only thing you need to get going is a dominance interaction matrix.
This is just a tabulation of dyadics dominance interactions where one individual is the winner and another individual is a loser. 
...
We start with an example of dominance interactions between seven badgers.
...
```{r, echo=FALSE, fig.width=6, fig.height=3.7, out.width="50%", fig.align="center", fig.cap="An example network of seven badgers."}
par(family = "serif")
...
Here we have `r sum(mat)` interactions so our rule of thumb suggests to use 20 randomizations.
If you want to speed up the calculations I would suggest (at least for the sake of this tutorial) to use a smaller value here, e.g. `n_rand = 3`, like so:
...
set.seed(123)
elo_res <- elo_steepness_from_matrix(mat = mat, refresh = 0, cores = 4)
saveRDS(elo_res, file = "vignette_data/elo_res_vignette.rds")
```

```{r, echo=FALSE}
elo_res <- readRDS("../vignette_data/elo_res_vignette.rds")
```
...
```{r, echo=2:2, fig.width=7, fig.height=4.7, out.width="70%", fig.align='center'}
par(family = "serif")
plot_steepness(elo_res)
```

The yellow area gives a visual summary of the posterior combined across all randomizations.
The finer gray lines (20 in my case, corresponding to 20 randomized sequences) represent the posteriors for each individual randomization.
...
I'd also note that the posterior is fairly wide, indicating that steepness in this example is unlikely to be greater than 0.8 or smaller than 0.5 (as seen by nearly no probability mass falling outside the interval between 0.5 and 0.8).
...
These quantiles show that only 4.5% of posterior samples provided steepness values below $`r sprintf("%.2f", p$q045[1])`$ and 4.5 % above $`r sprintf("%.2f", p$q955[1])`$ (the 89% credible interval).
This by and large confirms the visual assessment.
If you want to report interquartile-range, this is also included via the 25% and 75% quantiles ($`r sprintf("%.2f", p$q250[1])`$ and $`r sprintf("%.2f", p$q750[1])`$).
...
But then, what is actually also really interesting is to look at the underlying individual scores on which the steepness calculation is based.
...
And then use `plot_scores()` to visualise the posteriors.

```{r, echo=2:3, fig.width=7, fig.height=4.7, out.width="70%", fig.align='center'}
par(family = "serif")
...
There is one individual \textit{d}, which only has one observed interaction (a loss against \textit{f}).
If we focus on \textit{d} we see can several things.
...
For one, its posterior is pretty wide, ranging from almost 0 to about 5, with its 'peak' at about 2 (median is `r sprintf("%.2f", my_scores$median[my_scores$id == "d"])` to be precise).
And this makes sense, because we have very little data for \textit{d} (just one observation), which leads to the wide posterior.
Also, this one observation was a loss, which shifts the peak to below the average value (a naive view would suggest that an individual for which we have no knowledge is average).
The average is 3 in this case ($(n - 1)/2$), which you can verify by `mean(my_scores$mean)`.
The fact that \textit{d}'s peak is shifted fairly far below 3 is probably due to the loss of \textit{d} being against \textit{f}, which is a fairly low-rated individual itself (because it lost most of its interactions).

In contrast to \textit{d}, individual \textit{a}'s posterior is the narrowest of all individuals.
...
\textit{a} does, however, not have the largest number of observations as one might suspect.
But, \textit{a} never lost an interaction but won all of them, so it seems fair to say that we should expect a high rating for it (or rather cumulative winning probability) with fairly little uncertainty.

The second thing I wish to point in the example are individuals \textit{b} and \textit{c}. 
...
Their posteriors overlap extensively, although \textit{b} has a slightly higher mean/median.
The interesting thing here is that these two are close in rank/rating whatever way you look at it.
And their *dyadic* relationship is far from clear-cut (out of 8 interactions, \textit{b} won 5 and \textit{c} won 3), so it makes sense that these two overlap with a slight 'advantage' for \textit{b}.
...
It provides the individual posteriors alongside the steepness slope.
...
Coming back to individuals \textit{b} and \textit{c}, whose posteriors overlap substantially: there are a lot of posterior samples in which \textit{b} has rank 2, but there is also a substantial amount of samples in which \textit{c} is ranked second.
...
```{r, echo=FALSE, eval = FALSE}
set.seed(123)
elo_res1b <- elo_steepness_from_matrix(mat = mat1, refresh = 200, cores = 4)
saveRDS(elo_res1b, file = "vignette_data/elo_res1b_vignette.rds")
# takes about 7 minutes
```

```{r, echo=FALSE, eval = FALSE}
set.seed(123)
elo_res1c <- elo_steepness_from_matrix(mat = mat2, refresh = 200, cores = 4)
saveRDS(elo_res1c, file = "vignette_data/elo_res1c_vignette.rds")
# takes about 7 minutes
```


```{r, echo=FALSE}
elo_res_half <- readRDS(file = "../vignette_data/elo_res1b_vignette.rds")
elo_res_doubled <- readRDS(file = "../vignette_data/elo_res1c_vignette.rds")
```

Then we visualize the posteriors of steepness and the individual scores. 

```{r, echo=2:10, fig.width=9, fig.height=6.5, out.width="90%", fig.align='center'}
...
set.seed(123)
elo_res2a <- elo_steepness_from_matrix(mat = mat, refresh = 100, cores = 4)
saveRDS(elo_res2a, file = "vignette_data/elo_res2a_vignette.rds")

set.seed(123)
elo_res2b <- elo_steepness_from_matrix(mat = mat2, refresh = 100, cores = 4)
saveRDS(elo_res2b, file = "vignette_data/elo_res2b_vignette.rds")

set.seed(123)
elo_res2c <- elo_steepness_from_matrix(mat = mat5, refresh = 100, cores = 4)
saveRDS(elo_res2c, file = "vignette_data/elo_res2c_vignette.rds")
```

```{r, echo=FALSE}
elo_res2a <- readRDS(file = "../vignette_data/elo_res2a_vignette.rds")
elo_res2b <- readRDS(file = "../vignette_data/elo_res2b_vignette.rds")
elo_res2c <- readRDS(file = "../vignette_data/elo_res2c_vignette.rds")
```


```{r, eval = FALSE, echo=FALSE, fig.width=9, fig.height=6.5, out.width="90%", fig.align='center'}
par(mfrow = c(2, 3), family = "serif")
mycols <- hcl.colors(7, alpha = 0.7)
plot_steepness(elo_res2a)
plot_steepness(elo_res2b)
plot_steepness(elo_res2c)

plot_scores(elo_res2a, color = mycols)
plot_scores(elo_res2b, color = mycols)
plot_scores(elo_res2c, color = mycols)
```
...
set.seed(123)
elo_bonobos1 <- elo_steepness_from_matrix(mat = mat1, n_rand = 5, refresh = 100, cores = 4)
saveRDS(elo_bonobos1, file = "vignette_data/elo_bonobos1_vignette.rds")


set.seed(123)
elo_bonobos5 <- elo_steepness_from_matrix(mat = mat5, n_rand = 5, refresh = 100, cores = 4)
saveRDS(elo_bonobos5, file = "vignette_data/elo_bonobos5_vignette.rds")
```

```{r, eval = FALSE, echo=FALSE, fig.width=6, fig.height=6.5, out.width="70%", fig.align='center'}
elo_bonobos1 <- readRDS(file = "../vignette_data/elo_bonobos1_vignette.rds")
elo_bonobos5 <- readRDS(file = "../vignette_data/elo_bonobos5_vignette.rds")

par(mfrow = c(2, 2), family = "serif")
mycols <- hcl.colors(7, alpha = 0.7)
plot_steepness(elo_bonobos1)
plot_steepness(elo_bonobos5)

plot_scores(elo_bonobos1, color = mycols, adjustpar = 7)
plot_scores(elo_bonobos5, color = mycols, adjustpar = 7)
```
...
```{r, echo=FALSE, eval=FALSE}
set.seed(123)
m1 <- simple_steep_gen(6, 40, 0.9)$matrix
m2 <- m1 * 2
m5 <- m1 * 5
set.seed(123)
x <- elo_steepness_from_matrix(mat = m1, n_rand = 10, refresh = 100, cores = 4)
saveRDS(x, file = "vignette_data/elo_3a_vignette.rds")
set.seed(123)
x <- elo_steepness_from_matrix(mat = m2, n_rand = 10, refresh = 100, cores = 4)
saveRDS(x, file = "vignette_data/elo_3b_vignette.rds")
set.seed(123)
x <- elo_steepness_from_matrix(mat = m5, n_rand = 10, refresh = 100, cores = 4)
saveRDS(x, file = "vignette_data/elo_3c_vignette.rds")
```


```{r, echo=FALSE}
r1 <- readRDS(file = "../vignette_data/elo_3a_vignette.rds")
r2 <- readRDS(file = "../vignette_data/elo_3b_vignette.rds")
r5 <- readRDS(file = "../vignette_data/elo_3c_vignette.rds")
```


```{r, eval = TRUE, echo=2:15, fig.width=9, fig.height=6.5, out.width="90%", fig.align='center'}
par(mfrow = c(2, 3), family = "serif")
...
saveRDS(david_res, file = "vignette_data/david_res_vignette.rds")
# less than a minute
```

```{r, echo=FALSE}
david_res <- readRDS("../vignette_data/david_res_vignette.rds")
```
...
```{r david_steep_01, cache=TRUE, eval = FALSE}
david_res <- davids_steepness(mat, refresh = 0)
```
...
```{r, fig.height=4.5, fig.width=7, out.width="70%", fig.align='center', echo=2:2}
par(family = "serif")
