...
If you want to speed up the calculations I would suggest (at least for the sake of this tutorial) to use a smaller value here though, e.g. `n_rand = 3`, like so:

```{r elo_steep_01, eval = TRUE, cache=TRUE}
elo_res <- elo_steepness_from_matrix(mat = mat, n_rand = 3, refresh = 0, cores = 2)
```
...
These quantiles show that only 4.5% of posterior samples provided steepness values below $`r sprintf("%.2f", p$q045[1])`$ and 4.5 % above $`r sprintf("%.2f", p$q945[1])`$ (the 89% credible interval).
...
# knitr::kable(my_scores, format = "latex", digits = 3)
...
elo_res_half <- elo_steepness_from_matrix(mat = mat1, n_rand = 3, refresh = 0, cores = 2)
elo_res_doubled <- elo_steepness_from_matrix(mat = mat2, n_rand = 3, refresh = 0, cores = 2)
```
...
# plot(0, 0, "n", xlim = c(1, 3), ylim = c(0, 3.5))
# for (i in 1:nrow(x)) points(1:3, x[i, ], type = "l")
...
m5 <- m1 * 5
# calculate steepness
r1 <- elo_steepness_from_matrix(mat = m1, n_rand = 5, cores = 2)
r2 <- elo_steepness_from_matrix(mat = m2, n_rand = 5, cores = 2)
r5 <- elo_steepness_from_matrix(mat = m5, n_rand = 5, cores = 2)
```
...
To make this data set a bit more manageable (the full data set contains `r nrow(baboons1)` interactions between `r length(unique(c(baboons1$Winner, baboons1$Loser)))`), we reduce the original sequence to 200 interactions, which occur among `r length(unique(c(s$Winner, s$Loser)))` males.
...
```{r babsteep, echo = 2:10, cache=TRUE, eval=TRUE}
...
This document took `r round(as.numeric(difftime(time2 = compile_start, time1 = Sys.time(), units = "mins")), 1)` minutes to compile.
